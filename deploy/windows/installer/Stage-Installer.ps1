...
try {
  ...
  if ($IncludeOptionalBasemaps) {
    ...
    foreach ($file in @('california.pmtiles', 'buildings.pmtiles', 'buildings-overture.json', 'overlays.pmtiles', 'overlays-manifest.json', 'facilities.pmtiles', 'facilities-manifest.json', 'boundaries.pmtiles', 'boundaries-manifest.json', 'risk.pmtiles', 'risk-manifest.json')) {
