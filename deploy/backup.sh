#!/usr/bin/env bash
# OpenEOC backup (VEOC-40). The system of record is two parts: the database
# (boards, records, audit log, file metadata) AND the uploaded file blobs,
# which live on disk in the api container's OPENEOC_DATA_DIR, not in Postgres.
# A database-only dump restores file rows that point at missing bytes, so this
# writes both a database dump and a blob archive under one timestamp. Run on a
# schedule (schedule-backup.sh); keep both files together and off the box.
#
#   ./backup.sh [backup directory, default ./backups]
#
# After both files are written, backups in that directory older than
# OPENEOC_BACKUP_KEEP_DAYS days (default 14) are removed. A failed run removes
# nothing.
set -euo pipefail
# The files hold the whole database and every upload; keep them private.
umask 077
here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$here"

out_dir="${1:-./backups}"
keep_days="${OPENEOC_BACKUP_KEEP_DAYS:-14}"
[[ "$keep_days" =~ ^[1-9][0-9]*$ ]] || {
  echo "refusing: OPENEOC_BACKUP_KEEP_DAYS must be a whole number of days, 1 or more" >&2
  exit 2
}
mkdir -p "$out_dir"
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
db_file="$out_dir/openeoc-$stamp.sql.gz"
blob_file="$out_dir/openeoc-$stamp.blobs.tar.gz"

# Each file is written under a .part name and renamed only when its command
# succeeded, so a failed run never leaves a file that looks like a backup.
docker compose exec -T db pg_dump -U openeoc_owner -d openeoc --no-owner \
  | gzip > "$db_file.part"
mv "$db_file.part" "$db_file"
printf '[openeoc] database backup written: %s\n' "$db_file"

# Stream the blob directory out of the api container. The bytes are
# content-addressed and immutable, so a plain archive is a consistent copy.
# An upload in progress is a .upload-* staging file that is renamed when it
# completes; it is left out so the rename cannot fail the archive.
docker compose exec -T api tar -C /data/blobs --exclude='.upload-*' -cf - . \
  | gzip > "$blob_file.part"
mv "$blob_file.part" "$blob_file"
printf '[openeoc] blob backup written: %s\n' "$blob_file"

# Retention compares the timestamp in each name, so a file copied back in from
# elsewhere keeps its age. Files this script did not name are never touched.
cutoff="$(date -u -d "$keep_days days ago" +%Y%m%dT%H%M%SZ)"
for old in "$out_dir"/openeoc-*.sql.gz "$out_dir"/openeoc-*.blobs.tar.gz; do
  old_stamp="${old##*/openeoc-}"
  old_stamp="${old_stamp%%.*}"
  if [ -f "$old" ] && [[ "$old_stamp" =~ ^[0-9]{8}T[0-9]{6}Z$ ]] && [[ "$old_stamp" < "$cutoff" ]]; then
    rm -f -- "$old"
    printf '[openeoc] removed backup older than %s days: %s\n' "$keep_days" "$old"
  fi
done
