...
export function SmartFormsSurface(props: SmartFormsSurfaceProps) {
  ...
  const boards = useAsync(async () => {
    ...
    // A board whose record rules restrict this person still takes queued
    // reports: each is applied through those rules when it syncs (AG-07).
    return Promise.all(refs.map((ref) => props.client.getBoard(ref.id, incidentId)));
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (online) {
        ...
        if (synchronized.receipt?.late) setMessage(synchronized.message);
        else if (synchronized.phase === "synced") setMessage("Report synchronized with retained server attribution.");
      }
    } catch (reason) {
  ...
  return <Scroll>
    ...
    <section className="eoc-field-workspace" aria-label="Field report capture">
      ...
      <section className="eoc-field-selector" aria-labelledby="field-assignment-title">
        ...
        {props.onOpenMap ? <div className="eoc-field-map-link"><div><strong>Need map placement?</strong><span>{hasDraft ? "Queue or clear these answers before leaving this form." : "Place a point on the COP map. With no connection it is kept on this device and sent with the rest of the queue."}</span></div>
          <ActionButton kind="quiet" disabled={hasDraft} onClick={props.onOpenMap}><Icon name="map" decorative size={16} /> Open map capture</ActionButton></div> : null}
      </section>
