...
async function resolvePeriod(
  ...
  const triples = new Set(
    periods.map((period) => JSON.stringify([period.label, period.startsAt, period.endsAt]))
  )
  if (triples.size > 1)
...
async function recordIapAudit(
  ...
  input: {
    incidentId: string
    category: 'iap.assembled' | 'iap.submitted' | 'iap.ics204.revised' | 'iap.revision.created'
    subjectId: string
...
export async function createIap(
  ...
  await recordIapAudit(sql, actor, authority, {
    ...
    payload: {
      ...
      periodRevision: period?.revision ?? null,
      forms: iap.forms.length,
...
function requireIapEditAuthority(
  ...
  const participantOwn =
    authority.canContribute &&
    authority.participation !== null &&
  ...
  if (!hasOwnerWriteAuthority(actor, authority) && !participantOwn)
    throw new AuthError(
      403,
      "requires owner write authority or the current preparer's incident grant"
    )
}
...
export async function replaceIcs204Assignments(
  ...
  const assignments = await resolveIcs204Assignments(
    ...
    actor,
    row.incidentId,
    hasOwnerWriteAuthority(actor, authority)
      ? authority.jurisdictionId
      : row.preparedOrganizationId,
    input.assignments
  )
...
export async function submitIapForApproval(
  sql: Sql,
  actor: Principal,
  iapId: string
): Promise<void> {
...
export async function queryIapWorkspace(
  ...
  const byState = Object.fromEntries(
    states.map((state) => [state, iaps.filter((iap) => iap.status === state).length])
