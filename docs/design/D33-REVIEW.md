...
| 16 | Minor | Dock, board list, map layers, record drawer title | Incident boards are titled with template keys, such as "...: road_closures" and "...: resource_request" | Assigned to `86+D35`: the titles are set at activation on the server |
| 17 | Minor | Resources | Priority, state and next action show keys ("routine", "submitted", "triaged"); the board list shows "layer" | Assigned to `86+D35`: labels belong in the dictionary |
...
| 19 | Minor, outside the set | Chronology, Tasks, board designer (two tab sets) | Tabs name panels that do not exist, the defect fixed in finding 3 | Assigned to `86+D35`, with the same fix: make the switched region the tab panel |
| 20 | Minor | Context drawer on a phone | The drawer is an aside given the dialog role (axe moderate) | Assigned to `86+D35`: changing the element changes how its header is exposed, which the shell test checks |
...
| 22 | Minor | Context drawer | Closing the drawer on a phone saves "closed" as that layout's desktop preference | Assigned to `86+D35` |
| 23 | Minor | Loading notes | Text such as "Loading reports…" shows but is not announced | Assigned to `86+D35`: a status role on the shared note changes which element existing tests find as the page's status |
