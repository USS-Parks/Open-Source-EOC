...
| 17 | Where this session lands work | VA1 to VA12, VA37 and VA38 were landed by a cloud session through `claude/veoci-research-integration-n1kd0p`. On 2026-09-25 Basho stopped that session and moved the plan to a local session in the canonical checkout, which is now the integrating session. Each unit is committed on `main` there (or on a local `lane/*` branch under the standing grant, rebased on current `main`) and pushed once its gate is green; a rejected push means rebase and gate again, never a merge or a force. CI runs on the push to `main`; no workflow is dispatched by hand. |
| 18 | The Windows setup at phase ends | Built on the Windows machine that runs the plan, at each phase end. The cloud session's receipts before the move say it could not build it. |
| 19 | The test bed | A throwaway PostgreSQL 16.15 cluster with PostGIS 3.6.2, from the release build's runtime, on 127.0.0.1:55440 with durability settings off for speed, and Playwright's Chromium, on Windows. Receipts before the move name the cloud session's Linux bed (PostgreSQL 16 with PostGIS 3.4 on 127.0.0.1:55439). CI on Windows remains the platform check. |
