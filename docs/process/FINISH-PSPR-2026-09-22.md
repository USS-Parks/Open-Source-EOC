...
| `W3.12` | Screens for the operator routes the route-coverage test found with no caller, added during execution: notification rule authoring and the webhook allowlist, lifeline and messaging settings, libraries, checklist completion, dataset load, dashboard creation, corrective action detail, impact comparison, thread and dashboard-template export, and the damage baseline. Acceptance: each leaves the test's awaiting list | `W3.0` | the owning surfaces, client methods | M |
| `W3.13` | Screens for the optional integrations' operator routes, added during execution: collaboration backend and incident channels, meetings and briefings, facility status queries, tracked object detail, each shown only where its integration runs. Acceptance: each leaves the test's awaiting list | `W3.4` | new integration surfaces, client methods | M |
...
| `W4.11` | Public Assistance for F8, added during execution: a PA damage inventory by FEMA categories A to G, the per-capita indicator computed from PA cost once PA items exist, a shelter census from facilities reports where that integration runs, and both in the declaration summary. Acceptance: F8's engine boundary closes with a real-database and browser test | `W3.2` | `server/src/damage/**`, `web/src/damage/**`, `shared/src/dictionary/pda.ts`, one migration | M |
| `W4.12` | REST record writes through the sync hub, added during execution: every committed REST write of record data appears in the board's sync log, reaches live subscribers without a reconnect, and federates like a sync edit, with a stated ordering rule. Acceptance: a live socket sees a console edit and a peer receives it | `W4.1` | `server/src/sync/hub.ts`, `server/src/boards/service.ts`, one migration | M |
