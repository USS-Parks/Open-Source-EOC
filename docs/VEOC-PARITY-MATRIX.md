...
| F19 | NAPSG/DHS incident symbology shipped | Esri-EMO | Incident symbols render from a NAPSG set | Approved nine-type NAPSG subset, conservative mapping, independent status frames, local license/provenance and reproducible 1x/2x sprites integrated (`389afe1`); real MapLibre light/dark legend, inspection and offline proof passed. Unmapped broad clinic/airport/EOC source labels remain explicit | verified for agreed subset | Handoff 13; final COP composition P-COP |
...
| G-KPIMAP | Map-extent KPIs | Esri-EMO | Bounds callback (`6cb0500`) and uncapped incident-area intersect viewport queries, comparison and drilldown (`3b5cb38`) integrated with real-DB coverage/masking proof; KPI presentation remains P-COP | partial | Handoff 12 / P-COP |
| G-BUILDINGS | Enriched building subtypes (Overture) | Esri-EMO | H14 exact-way enrichment installed; real-map identity, status precedence and attribution proven in both themes; final inspection workspace remains P-COP | partial | H14 / P-COP |
