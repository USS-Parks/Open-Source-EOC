...
| F6 | Any board as a live geospatial layer; field-to-COP loop | Esri-EMO / WebEOC-Maps | Drop a record on the map; it appears as a COP layer | COP map + field capture (VEOC-16/17). Incident-scoped operational layers and dataset items on the COP still open | partial | VEOC-79B1/79C2 |
...
| F18 | Sensor and drone live feeds into the COP | Esri-EMO | External hazard/position feeds appear as COP layers | Feed framework: CAP/GeoRSS/CoT/GeoJSON, staleness, alarmed failure (VEOC-19). Durable dataset ingestion open | partial | VEOC-79C1/C2 |
...
| R3 | Agency/organization/volunteer conglomerate COP access | WebEOC-inc | Multiple organizations share one incident's COP | Named incident participation + authority + guest access (`ca0d130`, `16ce911`); incident-scoped operational records open | partial | VEOC-79B1/B2 |
...
| G-KPIMAP | Map-extent KPIs | Esri-EMO | Dashboard runtime filter + drilldown shipped (`7cf8791`); map-extent KPIs open | open | Handoff 12 |
...
| G-BOARDROUTE | Configurable board workflow routing | WebEOC-DS | Positions/lifecycle/notifications exist; declarative routing open | open | VEOC-81B |
| G-SHELL | Adaptable, responsive operator shell | Esri-EMO | Fixed layout; responsive/collapsible panels open (audit §5) | open | VEOC-81C |
